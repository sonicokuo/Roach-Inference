...
class ObsManager(ObsManagerBase):
    ...
    def attach_ego_vehicle(self, parent_actor):
        ...
        maps_h5_path = "/media/hcis-s09/pny_2tb/carla_13/PythonAPI/example_roach/carla_gym/core/obs_manager/birdview/maps/Town10HD.h5"

        #print(maps_h5_path)
        waypoints = self._world.get_map().generate_waypoints(2)
        margin = 100
        min_x = min(waypoints, key=lambda x: x.transform.location.x).transform.location.x - margin
        min_y = min(waypoints, key=lambda x: x.transform.location.y).transform.location.y - margin

        self._world_offset = np.array([min_x, min_y], dtype=np.float32)
        # print(world_offset)
    ...
    def get_observation(self):
        ...
        #comment all image parts because not they are used now
        ...
        masks = np.stack((c_route, c_route, c_route, *c_vehicle_history, *c_walker_history, *c_tl_history), axis=2)
    ...
    def _get_mask_from_stopline_vtx(self, stopline_vtx, M_warp):
        ...
        for sp_locs in stopline_vtx:
            ...
            # print(tuple(stopline_warped[0, 0].astype(int)), tuple(stopline_warped[1, 0].astype(int)))
            # print(type(stopline_warped[1, 0]))
            # print(type(stopline_warped[1, 0][0]))
            cv.line(mask, tuple(stopline_warped[0, 0].astype(int)), tuple(stopline_warped[1, 0].astype(int)),
                    color=1, thickness=6)
